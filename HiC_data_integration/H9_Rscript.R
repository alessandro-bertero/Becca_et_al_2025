...
if(alignment == TRUE){
  ...
  if(!"matrix" %in% colnames(table)) {
    ...
    dir.create("/home/shared_folder/matrix")
    for(i in nrow(table)){
      system2("wget", paste0("-P /home/shared_folder/matrix ", table$link[i]))
      system2("gzip", "-dk ", table$matrix[i], ".gz")
    }
...
if(normalization == TRUE){
  ...
  if(indexing == FALSE){
    ...
    if(length(unique(na.omit(settings$fasta))) != 1){
      stop("invalid argument for fasta")} else{fasta = settings$fasta[1]
      system2("wget", "-P /home/shared_folder/ https://hgdownload.cse.ucsc.edu/goldenpath/hg19/bigZips/hg19.fa.gz")
      system2("gzip", "-dk /home/shared_foler/hg19.fa.gz")}
  }
