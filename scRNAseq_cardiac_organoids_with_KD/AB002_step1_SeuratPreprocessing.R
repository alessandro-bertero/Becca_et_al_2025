...
sample_info = read.csv("/home/shared_folder/aggr_all_config.csv")
sample.names = sample_info$sample_id
sample_info$RXN = paste0(sample_info$day, "_", sample_info$replicate)
...
pre_selection = data.frame(summary(as.factor(data_seurat@meta.data$RXN)))
...
post_selection1 = data.frame(summary(as.factor(data_seurat@meta.data$RXN)))
...
QC_filtered2 %>%
  ggplot(aes(x = nCount_RNA, y = percent.mt, color = RXN)) +
  geom_point()
ggsave(filename = paste0(seurat_prefix, "MTPerCounts_filtered2.pdf"),
       width = 12, height = 8)

QC_filtered2 %>%
  ggplot(aes(x = nCount_RNA, y = percent.ribo, color = RXN)) +
  geom_point()
ggsave(filename = paste0(seurat_prefix, "RiboPerCounts_filtered2.pdf"),
       width = 12, height = 8)

QC_filtered2 %>%
  ggplot(aes(x = percent.mt, y = percent.ribo, color = RXN)) +
  geom_point()
...
post_selection2 = data.frame(summary(as.factor(data_seurat@meta.data$RXN)))

#results
results = cbind(pre_selection, post_selection1, post_selection2)
colnames(results) = c("pre_filtering", "filtered1", "filtered2")
...
as.ggplot(DimPlot(data_seurat_norm, dims = c(1,2), reduction = "pca", group.by = c("knockdown", "day"), split.by = "shRNA"))
